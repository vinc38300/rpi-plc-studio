...
#   d10= marche_chauffage         (DV.BP_Marche_Arret_Chauffage RF130)  ← câblé
#   d11= (non utilisé — forçage K4 géré par bloc XOR externe sur la sortie od2)
#
# Ports de SORTIE :
#   od1 = autorisation_chaudiere  → K1 (RF238)
#   od2 = circ_rehausse_on        → K4 (RF239)
#   od3 = demande de circulateur ESC Chaudière
...
auto_ecs = T_valid and dem_ecs and (not bouclage_ok_ecs) and (not v3v_sol)
if auto_ecs:    state["sr_s8"] = True
if not dem_ecs or v3v_sol: state["sr_s8"] = False   # reset si solaire actif
...
# ── SR_S10 : mini ballon ──────────────────────────────────────
#   SET   : dem_mini ET bouclage insuffisant ET solaire inactif
#   RESET : ballon plein (T_ball_h ≥ consigne) OU solaire actif
#           [FIX v7] solaire actif → on lui laisse charger les ballons,
#           inutile de lancer la chaudière en parallèle
if dem_mini and (not bouclage_ok_ecs) and (not v3v_sol): state["sr_s10"] = True
if T_ball_h >= T_cons_bal or v3v_sol:  state["sr_s10"] = False
...
# ECS (sr_s8, sr_s10) : fonctionne indépendamment de marche_chauffage
# Plancher (sr_s9)     : conditionné par marche_chauffage
auto_ecs_or_mini = state["sr_s8"] or state["sr_s10"]
auto_planch_ok   = state["sr_s9"] and marche_chauffage
auto_final = (auto_ecs_or_mini or auto_planch_ok or state.get("pulse3",0)>0)
auto_final = auto_final and (not alarme) and (not arret_force)
od1 = auto_final   # K1 — autorisation brûleur

# ── Circulateur réhausse K4 ─────────────────────────────────
# K4 = récupération chaleur résiduelle par inertie uniquement.
# od2 : récupération chaleur résiduelle (inertie chaudière).
# Le forçage manuel K4 est géré par bloc XOR externe sur od2 (RF239),
# comme K7/K8 (V3V plancher). Ne pas mettre de forcage ici.
bouclage_chaud_ECS = (T_bouclage >= (T_ball_h + 5.0))
bouclage_chaud_CH = T_bouclage >= (T_cons_dep) 
od2 = bouclage_chaud_ECS or bouclage_chaud_CH   # K4 — réhausse inertie (forçage via XOR externe)
od3 = od2 and bouclage_chaud_ECS # demande de circulateur ESC Chaudière
