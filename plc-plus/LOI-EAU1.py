...
if not bp_marche_general:
    ...
    if not arret_v3v:
        if   forcage_open  and not od5:               cmd_ouv = True;  cmd_fer = False
        elif forcage_close:                           cmd_ouv = False; cmd_fer = True
    ...
    # FIX: écriture explicite des od_refs (od_ref = RF* ne passe pas dans ancien serveur)
    # [nettoyé] od_ref gère RF218 #, 1.0 if od1 else 0.0)   # od1 circ plancher K9
    # [nettoyé] od_ref gère RF219 #, 1.0 if od2 else 0.0)   # od2 V3V ouvrir K7
    # [nettoyé] od_ref gère RF220 #, 1.0 if od3 else 0.0)   # od3 V3V fermer K8
    # [nettoyé] od_ref gère RF316 #, 1.0 if od4 else 0.0)   # od4 demande plancher → P9_CH.d3
