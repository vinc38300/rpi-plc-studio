...
class BlockEditor(QWidget):
    ...
    def _page_to_program(self, page: dict) -> list:
        ...
        def signal_ref(src_b):
            ...
            if t == "PYBLOCK":  # FIX: sorties OA1..OAN des PYBLOCK
                return p.get("oa1_ref") or p.get("reg_out", "RF0")
            return None
        ...
        def build_cond(src_b):
            ...
            if t in ("SR_R", "SR_S"):
                return {"type": "input", "ref": p.get("bit", "M0")}
            # CARITHM / PYBLOCK : sortie od{N} → référence booléenne
            if t in ("CARITHM", "PYBLOCK"):
                for _n in range(1, 9):
                    _rf = p.get(f"od{_n}_ref")
                    if _rf:
                        return {"type": "input", "ref": _rf}
            return None
        ...
        def resolve_bool_out(bid, port="Q", _visited=None):
            ...
            for w in wires:
                if w["src"]["bid"] == bid and w["src"]["port"] == port:
                    ...
                    # ── CARITHM / PYBLOCK : allouer un bit M temp ────────
                    # Le COIL/timer/logique amont écrira dans ce bit ;
                    # le CARITHM/PYBLOCK le lira via d{i}_ref.
                    if t in ("CARITHM", "PYBLOCK"):
                        return _alloc_mbit(bid, port)
        ...
        for b in sorted_b:
            ...
            if bt in ("AND","OR","NOT","XOR","INV","NAND","NOR"):
                ...
                insb,  _ = wire_src(bid, "IN")   # pour INV/NOT
                # signal_ref retourne RF* si la source est une AV/sonde analogique
                # bool_ref retourne DV/MEM/INPUT si la source est booléenne
                blk["in1"] = signal_ref(i1sb) or bool_ref(i1sb) or p.get("reg_a")
                blk["in2"] = signal_ref(i2sb) or bool_ref(i2sb) or p.get("reg_b")
                if bt in ("NOT","INV"):
                    blk["in1"] = signal_ref(insb) or bool_ref(insb) or blk.get("in1") or p.get("reg_a")
            ...
            # ── Variables / Persistance ───────────────────────────────────
                ...
                for i in range(1, n_in + 1):
                    ...
                    blk[f"in{i}_ref"] = bool_ref(isb)
                out1 = resolve_bool_out(bid, "O1")
                out2 = resolve_bool_out(bid, "O2")
            ...
            # ── CArithm ───────────────────────────────────────────────────
                ...
                for i in range(1, int(p.get("n_od", 0)) + 1):
                    ...
                    if isinstance(ref, int) and canvas_rf and str(canvas_rf).startswith("RF"):
                        blk[f"od{i}_ref"]  = canvas_rf   # RF pour RUNTIMCNT
                        blk[f"od{i}_gpio"] = ref          # GPIO pour l'actionneur
                    else:
