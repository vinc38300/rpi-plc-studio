...
function drawWire(w,sel){
  ...
  const digitalOn = sb&&sb.active;                          // signal numérique ON
...
function updateActiveStates(state){
  if(!state||!pg())return;
  _simState=state; // rendre accessible à drawBlock
  pg().blocks.forEach(b=>{
    const p=b.params||{};
    switch(b.type){
      // ── E/S GPIO ──────────────────────────────────────────────────
      ...
      case'PROG_H':{
        b.active=false;
        const page=pg();
        function traceActive(srcId,visited){
          ...
          for(const w of page.wires){
            ...
            if(['AND','OR','NOT','XOR','INV','NAND','NOR','COIL','SET','RESET','CARITHM','COMPARE','PYBLOCK','MOVE','COMPH','COMPL','HYST','COMPARE_F','SR','SR_R','SR_S','RS','GT','GE','LT','EQ','CONTACTOR','VALVE3V','GROUP_IN','GROUP_OUT','PLANCHER','CHAUDIERE','SOLAR','ZONE_CHAUF','ECS_BLOC','PROG_H'].includes(dst.type)){
              if(traceActive(dst.id,visited))return true;
            }
            // ── CONN/CONN_TX : traverser vers le CONN_RX/CONN pair de même num ──
            if(dst.type==='CONN'||dst.type==='CONN_TX'||dst.type==='CONN_RX'){
              // Continuer depuis le bloc lui-même (cas CONN avec OUT filaire)
              if(traceActive(dst.id,visited))return true;
              // Chercher le pair (récepteur) de même num et tracer depuis lui
              const rx=page.blocks.find(x=>
                (x.type==='CONN'||x.type==='CONN_RX')&&x.params.num===dst.params.num&&x.id!==dst.id);
              if(rx&&traceActive(rx.id,visited))return true;
            }
          }
          return false;
        }
        b.active=traceActive(b.id,new Set());
        break;
      }
      ...
      // ── CONN / CONN_TX / CONN_RX : traités APRÈS le forEach principal
      //    pour éviter le bug d'ordre (src.active pas encore calculé)
      case'CONN':
      case'CONN_TX':
      case'CONN_RX':{ b.active=false; break; } // réinitialisé, recalculé ci-dessous
  ...
  // ── CONN / CONN_TX / CONN_RX passe 1 (post-forEach) ─────────────────────
  // Tous les blocs sources (OR, AND, INV, COIL…) ont maintenant leur active correct.
  // On peut lire src.active en toute sécurité.
  const pageConn=pg();
  pageConn.blocks.filter(b=>b.type==='CONN'||b.type==='CONN_TX'||b.type==='CONN_RX').forEach(b=>{
    const inW=pageConn.wires.find(w=>w.dst.bid===b.id&&w.dst.port==='IN');
    if(inW){
      const src=pageConn.blocks.find(x=>x.id===inW.src.bid);
      b.active=src?!!src.active:false;
    }
    // sans fil IN → sera rempli en passe 2
  });

  // ── CONN / CONN_TX / CONN_RX passe 2 : propager l'état aux blocs sans fil entrant
  //    (le côté "sortie"/"RX" reçoit son état du côté "entrée"/"TX" même num)
  pageConn.blocks.filter(b=>b.type==='CONN'||b.type==='CONN_TX'||b.type==='CONN_RX').forEach(b=>{
    const inW=pageConn.wires.find(w=>w.dst.bid===b.id&&w.dst.port==='IN');
    if(!inW){
      // chercher un pair même num sur cette page avec fil entrant
      const peer=pageConn.blocks.find(x=>
        (x.type==='CONN'||x.type==='CONN_TX'||x.type==='CONN_RX')&&x.params.num===b.params.num&&x.id!==b.id&&
        pageConn.wires.some(w=>w.dst.bid===x.id&&w.dst.port==='IN'));
      if(peer) b.active=peer.active;
    }
  });
